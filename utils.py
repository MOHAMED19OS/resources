...
def ask(msg):
    ...
    while answer not in ['Y', 'N']:
        if not msg:
            print('\033[1K')
        answer = input(msg + (f"{bcolors.OKGREEN}Y{bcolors.ENDC}/{bcolors.WARNING}N{bcolors.ENDC}: ")).strip().upper()
        msg = ''
    return answer == 'Y'
...
def ReadUint64(tmp, le=True):
    if le:
        return ord(tmp[7]) << 56 | ord(tmp[6]) << 48 | ord(tmp[5]) << 40 | ord(tmp[4]) << 32 | ord(tmp[3]) << 24 | ord(tmp[2]) << 16 | ord(tmp[1]) << 8 | ord(tmp[0])
    else:
        return ord(tmp[0]) << 56 | ord(tmp[1]) << 48 | ord(tmp[2]) << 40 | ord(tmp[3]) << 32 | ord(tmp[4]) << 24 | ord(tmp[5]) << 16 | ord(tmp[6]) << 8 | ord(tmp[7])
...
def ReadElfHeader(file):
    ...
    tmp = file.read(4)
    if ELF_MAGIC != tmp:
        raise Exception(f'Wrong magic [{repr(tmp)}]!')

    tmp = ord(file.read(1))
    if tmp not in (ELFCLASS32, ELFCLASS64):
        raise Exception(f'Wrong elf class [{repr(tmp)}]!')
    ...
    if tmp not in (ET_EXEC, ET_DYN):
        raise Exception(f'Wrong type [{repr(tmp)}]!')
    ehdr['e_type'] = tmp
...
def GetElfAttributes(file, shdrTab, attribsId):
    ...
    if attribsId not in ('aeabi', 'gnu'):
        raise Exception(f'No supported attribs id: {attribsId}')
                ...
                while sectionLen > 0:
                    ...
                    while attrLen > 0 and p < len(contents):
                        ...
                        if tag == 1 and attrName == "gnu" and attribsId == "gnu":  # File Attributes
                            ...
                            attrLen = 0
            elif shrd['sh_type'] == SHT_MIPS_ABIFLAGS:
...
def GetPlatformInfo():
    ...
    try:
        ...
        if not glibcVersion:
            file = os.popen(f"{info['libc_path']} --version 2>&1")
        ...
        with open(info['libc_path'], "rb") as file:
            ...
            if EM_ARM == ehdr['e_machine']:
                ...
                if (ehdr['e_flags'] & EF_ARM_EABI_VER5) != EF_ARM_EABI_VER5:
                    raise Exception(f'ARM unsupported EABI [{repr(ehdr["e_flags"] & EF_ARM_EABIMASK)}]!')
                ...
                if (ehdr['e_flags'] & EF_ARM_ABI_FLOAT_HARD) == EF_ARM_ABI_FLOAT_HARD:
                    ...
                    raise Exception(f'Unknown ARM FPU ABI [{repr(ehdr["e_flags"])}]!')
                ...
                if (ehdr['e_flags'] & EF_SH_MACH_MASK) != EF_SH4:
                    raise Exception(f'Not supported SH ARCH [{repr(ehdr["e_flags"] & EF_SH_MACH_MASK)}]!')
                info['platform'] = 'sh4'
                info['fpu_type'] = 'hard'
                ...
                raise Exception(f"Not supported architecture: {repr(ehdr['e_machine'])}")
...
def getLibsPaths(installBase, platformInfo):
    libsPaths = [f'{installBase}usr/lib/', f'{installBase}lib/']
...
def getPackageConfig(platformInfo=None):
    ...
    installFPU = f'fpu_{fpuType}'

    return f'{e2iPlatform}_{installOld}{installFPU}'


def checkPyVersion():
    pyVersion = f"python{'.'.join(map(str, sys.version_info[:2]))}"
    if pyVersion not in ['python2.7', 'python2.6', 'python3.8', 'python3.9', 'python3.10', 'python3.11', 'python3.12']:
        printFatal(f'Your python version "{pyVersion}" is not supported!')
    return pyVersion


def downloadUrl(url, out):
    printMSG(f'Downloading "{url.split("?", 1)[0]}"', '{0}', bcolors.ENDC)

    wget = f'{INSTALL_BASE}usr/bin/wget'
    listToCheck = ['wget --no-check-certificate', 'wget']
    if os.path.isfile(wget):
        listToCheck.insert(0, f'{wget} --no-check-certificate')

    if '?' not in url:
        url += f'?_={str(time.time())}'
    ...
    for cmd in listToCheck:
        try:
            tmpCmd = f'{cmd} "{url}" -O "{out}" '
            if not DEBUG:
                tmpCmd += " 2>&1"
            printDBG(f'Try: {tmpCmd}')
            ...
            if ret in [0, None]:
                ...
                printDBG(f"Download using {cmd} failed with return code: {ret}")

        except Exception as e:
            printDBG(e)
    if errorCodes:
        printWRN("Download failed {}: {}".format(', '.join(errorCodes), ', '.join(errorMsg)), '{0}')
    return wget
...
def checkFreeSpace(requiredFreeSpaceMB, packageName, allowForce=True, ):
    ...
    printDBG(f"Free space {availSpaceMB} MB in rootfs")
    if availSpaceMB < requiredFreeSpaceMB:
        msg = f"Not enough disk space for installing {packageName}!\nAt least {requiredFreeSpaceMB} MB is required.\nYou have {availSpaceMB} MB free space in the rootfs.\nDo you want to continue anyway?"
        if not allowForce or not ask(msg):
            printFatal(f"Not enough disk space for installing {packageName}!\nAt least {requiredFreeSpaceMB} MB is required.")
...
def GetOpenSSLVer(platformInfo):
    ...
    for ver in ['1.1', '1.0.2', '1.0.0', '0.9.8']:
        ...
        for path in libsPaths:
            filePath = f'{path}libssl.so.{ver}'
            if os.path.isfile(filePath) and not os.path.islink(filePath):
                libsslExist = True
                libsslPath = path

            filePath = f'{path}libcrypto.so.{ver}'
            if os.path.isfile(filePath) and not os.path.islink(filePath):
                libcryptoExist = True
                libcryptoPath = path
    ...
    printDBG(f"OpenSSL SONAME VERSION [{e2iOpenSSLVer}]")
    if e2iOpenSSLVer == '1.0.2':
        linksTab = []
        symlinksText = []
        if not os.path.isfile(f'{libsslPath}libssl.so.{e2iOpenSSLVer}'):
            linksTab.append((f'{libsslPath}libssl.so.{e2iOpenSSLVer}', f'{libsslPath}libssl.so.1.0.0'))
            symlinksText.append('%s -> %s' % linksTab[-1])

        if not os.path.isfile(f'{libcryptoPath}libcrypto.so.{e2iOpenSSLVer}'):
            linksTab.append((f'{libcryptoPath}libcrypto.so.{e2iOpenSSLVer}', f'{libcryptoPath}libcrypto.so.1.0.0'))
            symlinksText.append('%s -> %s' % linksTab[-1])
        ...
        if len(linksTab):
            ...
            answer = ''
            while answer not in ['Y', 'N']:
                answer = input(MSG_FORMAT.format(msg) + "\nY/N: ").strip().upper()
                msg = ''
        ...
        with open(f'{libsslPath}libssl.so.{e2iOpenSSLVer}', "rb") as file:
            if b'OPENSSL_1.0.2' in file.read():
                e2iOpenSSLVer = '1.0.2'

    printDBG(f"OpenSSL VERSION [{e2iOpenSSLVer}]")
